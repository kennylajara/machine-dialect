...
class TestTypeNarrowing:
    ...
    def test_type_check_narrowing(self) -> None:
        ...
        entry.add_instruction(ConditionalJump(is_int, "int_branch", "other_branch"))
        ...
        int_branch.add_instruction(BinaryOp(result, "+", x, Constant(10, MIRType.INT)))
    ...
    def test_type_assert_narrowing(self) -> None:
        ...
        entry.add_instruction(BinaryOp(result, "*", casted, Constant(2.0, MIRType.FLOAT)))
        entry.add_instruction(Return(result))
    ...
    def test_nested_type_checks(self) -> None:
        ...
        entry.add_instruction(BinaryOp(not_bool, "==", is_bool, Constant(False, MIRType.BOOL)))
        entry.add_instruction(ConditionalJump(not_bool, "not_bool", "is_bool"))
        ...
        not_bool_block.add_instruction(ConditionalJump(is_int, "is_int", "is_string"))
        ...
        is_int_block.add_instruction(BinaryOp(result_int, "+", x, Constant(1, MIRType.INT)))
        is_int_block.add_instruction(Return(result_int))
        ...
        is_string_block.add_instruction(Return(result_string))

        # Is bool branch
        is_bool_block = BasicBlock("is_bool")
        is_bool_block.label = "is_bool"
        result_bool = Constant(-1, MIRType.INT)
        is_bool_block.add_instruction(Return(result_bool))
    ...
    def test_union_type_cast_elimination(self) -> None:
        ...
        entry.add_instruction(ConditionalJump(is_int, "handle_int", "handle_float"))
        ...
        handle_int.add_instruction(BinaryOp(doubled, "*", int_val, Constant(2, MIRType.INT)))
        handle_int.add_instruction(Jump("done"))
        ...
        handle_float.add_instruction(BinaryOp(halved, "/", float_val, Constant(2.0, MIRType.FLOAT)))
        handle_float.add_instruction(Jump("done"))
        ...
        done.add_instruction(Return(result))
    ...
    def test_type_narrowing_with_copy(self) -> None:
        ...
        entry.add_instruction(Copy(y, x))  # y should inherit INT type
        ...
        # Use y as INT
        result = Temp(MIRType.INT, 1)
        entry.add_instruction(BinaryOp(result, "+", y, Constant(5, MIRType.INT)))
        entry.add_instruction(Return(result))
